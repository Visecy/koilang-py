...
class KoiLang(CommandSet, metaclass=KoiLangMeta):
    ...
    def add_handler(
        self, handler: Union[Type["AbstractHandler"], "AbstractHandler"]
    ) -> "AbstractHandler":
    ...
    def __parse(self, __lexer: BaseLexer, *, close_lexer: bool = True) -> None:
        # Directly use the lexer as input to Parser since it now implements IO[str]
            ...
            with self.exec_block():
                ...
                while True:
                    try:
                        ...
                        break
                    except (KoiLangError, KoiParseError):
    ...
    def __parse_and_ret(
        self, __lexer: BaseLexer, *, close_lexer: bool = True
    ...
    def parse(
        ...
        *,
        ...
        if isinstance(lexer, str):
            # Create a StringLexer from the string and use it as input to Parser
            lexer_obj = StringLexer(lexer, encoding=self.__class__.__text_encoding__)
            # For string lexers, we always close them
            close_lexer = True
        else:
            # Use the provided lexer directly
            lexer_obj = lexer

        # Delegate to the appropriate parse method
        if with_ret:
            return self.__parse_and_ret(lexer_obj, close_lexer=close_lexer)
        else:
            self.__parse(lexer_obj, close_lexer=close_lexer)
    ...
    def parse_file(
        ...
        **kwds: Any,
        ...
        parse a kola file.
        """
        # Pass path directly to Parser since it accepts paths
        f = FileLexer(path, encoding=encoding)
        return self.parse(f, **kwds)
    ...
    def __kola_caller__(
        self, command: Command, args: tuple, kwargs: Dict[str, Any], **kwds: Any
    ) -> Any:
    ...
    def on_exception(
        self,
        exc_type: Type[KoiLangError],
        exc_ins: Optional[KoiLangError],
